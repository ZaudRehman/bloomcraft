...
//!
//! - **Traits**: `BloomFilter`, `DeletableBloomFilter`, `MergeableBloomFilter`, `ScalableBloomFilter`
...
mod tests {
    ...
    fn test_validate_params_edge_cases() {
        ...
        // Invalid: load factor > 2.0 (updated from original)
        assert!(validate_params(100, 250, 7).is_err());
    }

    #[test]
    fn test_validate_params_allows_moderate_saturation() {
        // Load factor between 1.0 and 2.0 should be allowed (NEW TEST)
